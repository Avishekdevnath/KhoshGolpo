...
export function SiteFooter() {
  return (
    <footer className="border-t border-slate-800/60 bg-gradient-to-br from-slate-950/80 via-slate-950 to-blue-950/80 py-16">
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-12 px-4 sm:px-6 lg:px-8">
        <div className="grid gap-8 lg:grid-cols-[1.3fr_1fr_1fr_1fr]">
          <div className="space-y-4">
            <Link href="/" className="flex items-center gap-3 text-sm font-semibold tracking-[0.32rem] text-slate-200">
          ...
          {footerLinks.map((group) => (
            <div key={group.title} className="space-y-4">
              <h3 className="text-xs font-semibold uppercase tracking-[0.28rem] text-slate-500">{group.title}</h3>
        ...
        <div className="flex flex-col gap-4 text-xs uppercase tracking-[0.28rem] text-slate-500 sm:flex-row sm:items-center sm:justify-between">

"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { useForm } from "react-hook-form";
import { zodResolver } from "@hookform/resolvers/zod";
import { z } from "zod";
import { Eye, EyeOff } from "lucide-react";

import { useAuth } from "@/lib/auth/hooks";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Label } from "@/components/ui/label";
import { FormMessage } from "@/components/ui/form-message";
import { getErrorMessage } from "@/lib/utils/error";
import { appConfig } from "@/lib/config/env";

const registrationSchema = z
  .object({
    email: z.string().email("Enter a valid email address."),
    handle: z
      .string()
      .min(3, "Handle must be at least 3 characters.")
      .max(20, "Handle must be at most 20 characters.")
      .regex(/^[a-z0-9_.-]+$/i, "Handle can contain letters, numbers, underscores, dots or dashes."),
    displayName: z.string().max(80, "Display name must be at most 80 characters.").optional().or(z.literal("")),
    password: z.string().min(8, "Password must be at least 8 characters."),
    confirmPassword: z.string().min(8, "Please re-enter your password."),
  })
  .refine((data) => data.password === data.confirmPassword, {
    message: "Passwords must match.",
    path: ["confirmPassword"],
  });

const otpSchema = z.object({
  otp: z
    .string()
    .trim()
    .regex(/^\d{4,8}$/, "Enter the 4–8 digit code from your email."),
});

type RegisterFormValues = z.infer<typeof registrationSchema>;
type OtpFormValues = z.infer<typeof otpSchema>;

const inputClasses =
  "border-slate-800/70 bg-slate-900/70 text-slate-100 placeholder:text-slate-500 shadow-inner shadow-slate-900/60 focus-visible:ring-emerald-400/40";

export function RegisterForm() {
  const router = useRouter();
  const { register: registerAccount, verifyEmail, isActionPending } = useAuth();
  const [rootError, setRootError] = useState<string | null>(null);
  const [showPassword, setShowPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);
  const [verificationInfo, setVerificationInfo] = useState<{
    tokenId: string;
    expiresAt: string;
    email: string;
  } | null>(null);

  const {
    register,
    handleSubmit,
    formState: { errors, isSubmitting },
    setError,
    reset,
  } = useForm<RegisterFormValues>({
    resolver: zodResolver(registrationSchema),
    defaultValues: {
      email: "",
      handle: "",
      displayName: "",
      password: "",
      confirmPassword: "",
    },
  });

  const {
    register: registerOtpField,
    handleSubmit: handleSubmitOtp,
    formState: { errors: otpErrors, isSubmitting: isVerifying },
    setError: setOtpError,
    reset: resetOtpForm,
  } = useForm<OtpFormValues>({
    resolver: zodResolver(otpSchema),
    defaultValues: { otp: "" },
  });

  const expiresDisplay = (() => {
    if (!verificationInfo?.expiresAt) return null;
    const date = new Date(verificationInfo.expiresAt);
    if (Number.isNaN(date.getTime())) return verificationInfo.expiresAt;
    return date.toLocaleString();
  })();

  const handleRegisterSubmit = handleSubmit(async (formValues) => {
    setRootError(null);
    try {
      const payload = {
        email: formValues.email,
        handle: formValues.handle,
        password: formValues.password,
        displayName: formValues.displayName?.trim() ? formValues.displayName.trim() : undefined,
      };
      const result = await registerAccount(payload);
      if (result.emailVerificationRequired) {
        setVerificationInfo({
          tokenId: result.verificationTokenId,
          expiresAt: result.verificationExpiresAt,
          email: payload.email,
        });
        resetOtpForm();
      } else {
        router.replace(appConfig.defaultRedirect);
        router.refresh();
      }
    } catch (error) {
      const message = getErrorMessage(error, "Unable to create your account.");
      setRootError(message);
      setError("email", { message: "Check if the email is already registered." });
    }
  });

  const handleVerifySubmit = handleSubmitOtp(async ({ otp }) => {
    if (!verificationInfo) {
      setRootError("Unable to verify email. Please try signing up again.");
      return;
    }

    try {
      setRootError(null);
      await verifyEmail({ tokenId: verificationInfo.tokenId, otp });
      reset();
      resetOtpForm();
      setVerificationInfo(null);
      router.replace(appConfig.defaultRedirect);
      router.refresh();
    } catch (error) {
      const message = getErrorMessage(error, "Unable to verify your email. Check the code and try again.");
      setRootError(message);
      setOtpError("otp", { message: "Code invalid or expired. Re-enter the latest code sent to you." });
    }
  });

  const pending = verificationInfo ? isVerifying || isActionPending : isSubmitting || isActionPending;
  const isOtpStep = Boolean(verificationInfo);

  return (
    <div className="space-y-6">
      <div className="space-y-2">
        <h2 className="text-xl font-semibold text-white">{isOtpStep ? "Verify your email" : "Create your account"}</h2>
        <p className="text-sm text-slate-400">
          {isOtpStep
            ? "Enter the verification code we just sent to your inbox to finish setting up your account."
            : "Sign up to start new conversations and keep your community close."}
        </p>
      </div>

      {!isOtpStep ? (
        <form className="space-y-6" onSubmit={handleRegisterSubmit} noValidate>
          <div className="space-y-2">
            <Label htmlFor="email" className="text-slate-300">
              Email
            </Label>
            <Input id="email" type="email" autoComplete="email" className={inputClasses} {...register("email")} />
            <FormMessage message={errors.email?.message} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="handle" className="text-slate-300">
              Handle
            </Label>
            <Input id="handle" autoComplete="username" className={inputClasses} {...register("handle")} />
            <FormMessage message={errors.handle?.message} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="displayName" className="text-slate-300">
              Display name (optional)
            </Label>
            <Input id="displayName" autoComplete="name" className={inputClasses} {...register("displayName")} />
            <FormMessage message={errors.displayName?.message} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="password" className="text-slate-300">
              Password
            </Label>
            <div className="relative">
              <Input
                id="password"
                type={showPassword ? "text" : "password"}
                autoComplete="new-password"
                className={`${inputClasses} pr-11`}
                {...register("password")}
              />
              <Button
                type="button"
                variant="ghost"
                size="icon"
                className="absolute inset-y-0 right-1 my-1 size-8 rounded-lg bg-slate-900/60 text-slate-300 hover:bg-slate-900/80 hover:text-white"
                onClick={() => setShowPassword((prev) => !prev)}
                aria-label={showPassword ? "Hide password" : "Show password"}
              >
                {showPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
              </Button>
            </div>
            <FormMessage message={errors.password?.message} />
          </div>

          <div className="space-y-2">
            <Label htmlFor="confirmPassword" className="text-slate-300">
              Re-enter password
            </Label>
            <div className="relative">
              <Input
                id="confirmPassword"
                type={showConfirmPassword ? "text" : "password"}
                autoComplete="new-password"
                className={`${inputClasses} pr-11`}
                {...register("confirmPassword")}
              />
              <Button
                type="button"
                variant="ghost"
                size="icon"
                className="absolute inset-y-0 right-1 my-1 size-8 rounded-lg bg-slate-900/60 text-slate-300 hover:bg-slate-900/80 hover:text-white"
                onClick={() => setShowConfirmPassword((prev) => !prev)}
                aria-label={showConfirmPassword ? "Hide confirm password" : "Show confirm password"}
              >
                {showConfirmPassword ? <EyeOff className="size-4" /> : <Eye className="size-4" />}
              </Button>
            </div>
            <FormMessage message={errors.confirmPassword?.message} />
          </div>

          <FormMessage message={rootError ?? errors.root?.message} />

          <Button
            type="submit"
            className="w-full rounded-xl bg-gradient-to-r from-indigo-500 to-sky-500 text-white shadow-lg shadow-sky-500/30 hover:from-indigo-400 hover:to-sky-500"
            isLoading={pending}
          >
            Create account
          </Button>
        </form>
      ) : (
        <form className="space-y-6" onSubmit={handleVerifySubmit} noValidate>
          <div className="rounded-2xl border border-slate-800/70 bg-slate-900/60 p-4 text-sm text-slate-300">
            <p>
              We sent a verification code to{" "}
              <span className="font-medium text-white">{verificationInfo?.email ?? "your inbox"}</span>.
            </p>
            <p className="mt-1 text-xs text-slate-400">
              {expiresDisplay
                ? `The code expires at ${expiresDisplay}. After that, sign up again to receive a new code.`
                : "The code expires soon. After it expires, sign up again to receive a new code."}
            </p>
          </div>

          <div className="space-y-2">
            <Label htmlFor="otp" className="text-slate-300">
              Verification code
            </Label>
            <Input
              id="otp"
              inputMode="numeric"
              autoComplete="one-time-code"
              placeholder="Enter the 4–8 digit code"
              maxLength={8}
              className={`${inputClasses} text-center text-lg tracking-[0.4em]`}
              {...registerOtpField("otp")}
            />
            <FormMessage message={otpErrors.otp?.message} />
          </div>

          <FormMessage message={rootError ?? otpErrors.root?.message} />

          <Button
            type="submit"
            className="w-full rounded-xl bg-gradient-to-r from-indigo-500 to-sky-500 text-white shadow-lg shadow-sky-500/30 hover:from-indigo-400 hover:to-sky-500"
            isLoading={pending}
          >
            Verify email
          </Button>

          <p className="text-xs text-slate-500">
            Didn’t get the code? Check your spam folder or{" "}
            <button
              type="button"
              className="cursor-pointer font-semibold text-sky-300 transition hover:text-sky-200"
              onClick={() => {
                setVerificationInfo(null);
                resetOtpForm();
                setRootError(null);
              }}
            >
              try signing up again
            </button>
            .
          </p>
        </form>
      )}
    </div>
  );
}


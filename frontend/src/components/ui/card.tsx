...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "flex flex-col gap-6 rounded-2xl border border-border/60 bg-card px-6 py-6 text-card-foreground shadow-lg shadow-slate-200/40 backdrop-blur-sm transition-colors dark:shadow-slate-950/40",
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("flex items-center justify-between pt-4 text-sm text-muted-foreground", className)}
      {...props}
    />
  );
}
